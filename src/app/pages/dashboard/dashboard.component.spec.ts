...
describe('DashboardComponent', () => {
  ...
  describe('Product simulation', () => {
      it('should be a button that subtracts in one the R of all products listed products', fakeAsync(() => {
        ...
        const closeButton: HTMLButtonElement = fixture.debugElement.query(By.css('.btn-primary')).nativeElement;
        closeButton.click();
        flush();
      ...
      it('should degrades at twice the speed the Q product when once the R drops below', fakeAsync(() => {
        ...
        component.products[0].sellIn = 0;
        fixture.detectChanges();

        const [breadQuality] = component.products.filter((product) => { return product.id === 11 }).map((product) => product.quality - 2);
        const closeButton: HTMLButtonElement = fixture.debugElement.query(By.css('.btn-primary')).nativeElement;
        closeButton.click();
        flush();
      ...
      it('should lowers the Q 1 unit per day when R is greater or equal to 0', fakeAsync(() => {
        component.products[0].sellIn = 1;
        fixture.detectChanges();
        const [breadQuality] = component.products.filter((product) => { return product.id === 11 }).map((product) => product.quality - 1);
        const closeButton: HTMLButtonElement = fixture.debugElement.query(By.css('.btn-primary')).nativeElement;
        closeButton.click();
        flush();
      ...
      it('show your tab in red when a product drops its quality to 0', fakeAsync(() => {
        ...
        component.products[2].quality = 1;
        fixture.detectChanges();
